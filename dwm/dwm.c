...
static const char autostartdir[] = ".config/dwm/scripts/autostart";
...
runautostart(void)
{
	 ...
	 * otherwise use ~/.config/dwm/scripts/autostart as autostart script directory
	 */
	...
	if (xdgdatahome != NULL && *xdgdatahome != '\0') {
		...
		/* space for path segments, separators and nul */
		pathpfx = ecalloc(1, strlen(home) + strlen(autostartdir) + 2);
...
centeredmaster(Monitor *m)
{
	...
	if (i < m->nmaster) {
		 ...
		 * of the screen */
		h = (m->wh - my) / (MIN(n, m->nmaster) - i);
		resize(c, m->wx + mx, m->wy + my, mw - (2*c->bw) + (n > 1 ? gappx : 0),
		       h - (2*c->bw), 0);
		my += HEIGHT(c);
	} else {
		...
		if ((i - m->nmaster) % 2 ) {
			h = (m->wh - ety) / ( (1 + n - i) / 2);
			resize(c, m->wx, m->wy + ety, tw - (2*c->bw) + (n > 1 ? gappx : 0),
			       h - (2*c->bw), 0);
			ety += HEIGHT(c);
		} else {
			h = (m->wh - oty) / ((1 + n - i) / 2);
...
grid(Monitor *m) {
	...
	for(i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
		...
		ah = ((i + 1) % rows == 0) ? m->wh - ch * rows : gappx; // Ajusted, so gaps match other layouts
		aw = (i >= rows * (cols - 1)) ? m->ww - cw * cols : gappx;
